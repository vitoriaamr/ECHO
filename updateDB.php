<?php
	...
	if($thisMD5 != $prevMD5) {
		// set up tables
		setupTable('circle_members', []);

		setupTable('circles', []);

		setupTable('followers', []);

		setupTable('friends', []);

		setupTable('messages', []);

		setupTable('password_resets', []);

		setupTable('post_comments', []);

		setupTable('post_likes', []);

		setupTable('post_saves', []);

		setupTable('post_shares', []);

		setupTable('posts', [
				" ALTER TABLE `posts` CHANGE `visibility` `visibility` TEXT NULL ",
				"ALTER TABLE posts ADD `field1` VARCHAR(40)",
				"ALTER TABLE `posts` CHANGE `field1` `visibility` VARCHAR(40) NULL ",
				" ALTER TABLE `posts` CHANGE `visibility` `visibility` TEXT NULL ",
				" ALTER TABLE `posts` CHANGE `visibility` `visibility` DATETIME NULL ",
				" ALTER TABLE `posts` CHANGE `visibility` `visibility` VARCHAR(40) NULL ",
				" ALTER TABLE `posts` CHANGE `visibility` `visibility` VARCHAR(20) NULL ",
				" ALTER TABLE `posts` CHANGE `visibility` `visibility` VARCHAR(20) NULL DEFAULT 'public' ",
			]);
