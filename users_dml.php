...
function users_insert(&$error_message = '') {
	...
	$data = [
		'handle' => Request::val('handle', ''),
		'name' => Request::val('name', ''),
		'password_hash' => Request::val('password_hash', ''),
		'bio' => Request::val('bio', ''),
		'city' => Request::val('city', ''),
		'avatar' => Request::val('avatar', ''),
	...
	$recID = tableInsert('users', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		users_copy_children($recID, Request::val('SelectedID'));
...
function users_update(&$selected_id, &$error_message = '') {
	...
	if($data['handle'] === '') {
		echo StyleSheet() . "\n\n<div class=\"alert alert-danger\">{$Translation['error:']} 'Handle': {$Translation['field not null']}<br><br>";
	...
	if($data['password_hash'] === '') {
		echo StyleSheet() . "\n\n<div class=\"alert alert-danger\">{$Translation['error:']} 'Password hash': {$Translation['field not null']}<br><br>";
		echo '<a href="" onclick="history.go(-1); return false;">' . $Translation['< back'] . '</a></div>';
		exit;
	}
	if($data['created_at'] === '') {
	...
	if(function_exists('users_after_update')) {
		if($row = getRecord('users', $data['selectedID'])) $data = array_map('makeSafe', $row);
...
function users_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('users', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('users', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('users', $selectedId, 'view'))
	...
	if($hasSelectedId) {
		if(!($row = getRecord('users', $selectedId))) {
	...
	if(!$fieldsAreEditable) {
		$jsReadOnly = '';
		$jsReadOnly .= "\t\$j('#handle').replaceWith('<div class=\"form-control-static\" id=\"handle\">' + (\$j('#handle').val() || '') + '</div>');\n";
		$jsReadOnly .= "\t\$j('#name').replaceWith('<div class=\"form-control-static\" id=\"name\">' + (\$j('#name').val() || '') + '</div>');\n";
		$jsReadOnly .= "\t\$j('#password_hash').replaceWith('<div class=\"form-control-static\" id=\"password_hash\">' + (\$j('#password_hash').val() || '') + '</div>');\n";
		$jsReadOnly .= "\t\$j('#bio').replaceWith('<div class=\"form-control-static\" id=\"bio\">' + (\$j('#bio').val() || '') + '</div>');\n";
		$jsReadOnly .= "\t\$j('#city').replaceWith('<div class=\"form-control-static\" id=\"city\">' + (\$j('#city').val() || '') + '</div>');\n";
		$jsReadOnly .= "\t\$j('#avatar').replaceWith('<div class=\"form-control-static\" id=\"avatar\">' + (\$j('#avatar').val() || '') + '</div>');\n";
	...
	$templateCode = str_replace('<%%UPLOADFILE(handle)%%>', '', $templateCode);
	$templateCode = str_replace('<%%UPLOADFILE(name)%%>', '', $templateCode);
	$templateCode = str_replace('<%%UPLOADFILE(password_hash)%%>', '', $templateCode);
	$templateCode = str_replace('<%%UPLOADFILE(bio)%%>', '', $templateCode);
	$templateCode = str_replace('<%%UPLOADFILE(city)%%>', '', $templateCode);
	$templateCode = str_replace('<%%UPLOADFILE(avatar)%%>', '', $templateCode);
	...
	if($hasSelectedId) {
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(handle)%%>', safe_html($urow['handle']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(handle)%%>', html_attr($row['handle']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(handle)%%>', urlencode($urow['handle']), $templateCode);
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(password_hash)%%>', safe_html($urow['password_hash']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(password_hash)%%>', html_attr($row['password_hash']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(password_hash)%%>', urlencode($urow['password_hash']), $templateCode);
		if( $dvprint) $templateCode = str_replace('<%%VALUE(bio)%%>', safe_html($urow['bio']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(bio)%%>', html_attr($row['bio']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(bio)%%>', urlencode($urow['bio']), $templateCode);
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(avatar)%%>', safe_html($urow['avatar']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(avatar)%%>', html_attr($row['avatar']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(avatar)%%>', urlencode($urow['avatar']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(handle)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(handle)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(name)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(name)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(password_hash)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(password_hash)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(bio)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(bio)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(city)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(city)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(avatar)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(avatar)%%>', urlencode(''), $templateCode);
	...
	$lookupFields = getLookupFields()['users'] ?? [];
	...
	if($hasSelectedId) {
		$jdata = get_joined_record('users', $selectedId);
	...
	if(function_exists('users_dv')) {
		$args = [];
		users_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
