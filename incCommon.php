<?php
	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'circle_members' => "`circle_members`.`id` as 'id', `circle_members`.`circle_id` as 'circle_id', `circle_members`.`user_id` as 'user_id', `circle_members`.`added_at` as 'added_at'",
			'circles' => "`circles`.`id` as 'id', `circles`.`owner_id` as 'owner_id', `circles`.`name` as 'name', `circles`.`created_at` as 'created_at'",
			'followers' => "`followers`.`id` as 'id', `followers`.`follower_id` as 'follower_id', `followers`.`following_id` as 'following_id', `followers`.`created_at` as 'created_at'",
			'friends' => "`friends`.`id` as 'id', `friends`.`user1_id` as 'user1_id', `friends`.`user2_id` as 'user2_id', `friends`.`created_at` as 'created_at'",
			'messages' => "`messages`.`id` as 'id', `messages`.`sender_id` as 'sender_id', `messages`.`receiver_id` as 'receiver_id', `messages`.`body` as 'body', `messages`.`created_at` as 'created_at'",
			'password_resets' => "`password_resets`.`id` as 'id', `password_resets`.`user_id` as 'user_id', `password_resets`.`new_password_hash` as 'new_password_hash', `password_resets`.`requested_at` as 'requested_at'",
			'post_comments' => "`post_comments`.`id` as 'id', `post_comments`.`user_id` as 'user_id', `post_comments`.`post_id` as 'post_id', `post_comments`.`body` as 'body', `post_comments`.`created_at` as 'created_at'",
			'post_likes' => "`post_likes`.`id` as 'id', `post_likes`.`user_id` as 'user_id', `post_likes`.`post_id` as 'post_id', `post_likes`.`created_at` as 'created_at'",
			'post_saves' => "`post_saves`.`id` as 'id', `post_saves`.`user_id` as 'user_id', `post_saves`.`post_id` as 'post_id', `post_saves`.`created_at` as 'created_at'",
			'post_shares' => "`post_shares`.`id` as 'id', `post_shares`.`user_id` as 'user_id', `post_shares`.`post_id` as 'post_id', `post_shares`.`created_at` as 'created_at'",
			'posts' => "`posts`.`id` as 'id', `posts`.`author_id` as 'author_id', `posts`.`content` as 'content', `posts`.`circle_id` as 'circle_id', `posts`.`created_at` as 'created_at', `posts`.`visibility` as 'visibility'",
			'users' => "`users`.`id` as 'id', `users`.`handle` as 'handle', `users`.`name` as 'name', `users`.`password_hash` as 'password_hash', `users`.`bio` as 'bio', `users`.`city` as 'city', `users`.`avatar` as 'avatar', `users`.`created_at` as 'created_at'",
		];
...
EOT;
	...
	function isDetailViewEnabled($tn) {
		$tables = ['circle_members', 'circles', 'followers', 'friends', 'messages', 'password_resets', 'post_comments', 'post_likes', 'post_saves', 'post_shares', 'posts', 'users', ];
